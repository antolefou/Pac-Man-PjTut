...
public class FantomeGroup extends Group {
    ...
    public void initNumFantome() {
        for (int i=0; i<fantomes.length; i++) {
            ...
            fantomes[i].imageFantomeGele = new Image(Objects.requireNonNull(getClass().getResourceAsStream("ghost" + fantomes[i].numFantome + "Gele.gif")));
        }
    }
        ...
        for (Fantome fantome : fantomes) {
            switch (fantome.etat) {
                ...
                case APPEURE:
                    ...
                    break;
                default:
                    fantome.setImageView(new Image(Objects.requireNonNull(getClass().getResourceAsStream("ghost" + fantome.numFantome + ".gif"))));
                    break;
            }
        }
    }
